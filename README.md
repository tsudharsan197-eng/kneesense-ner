...
- [`firmware/kneesense_esp32/kneesense_esp32.ino`](firmware/kneesense_esp32/kneesense_esp32.ino) — **single-sensor mode, MPU6050 only**: one MPU6050 (shin-mounted) → complementary filter → BLE notify. No calibration button, RGB LED, buzzer, or vibration motor on this board (the original hardware plan's feedback pieces were dropped) — calibration is triggered purely over BLE from the app, and the app's own UI (`SensorPairingPage`, `ErrorModal`) is the only status/error feedback there is. The MPU6050 itself is driven by direct I2C register access rather than the Adafruit_MPU6050 library, to skip that library's init overhead — see the `.ino`'s "MPU6050 direct register access" section for exactly what it replicates (wake-from-sleep, range/filter config, the WHO_AM_I check) to keep the angle math numerically identical. The hardware moved from an earlier two-sensor (thigh+shin) design to one sensor; the app's `bleConnection.ts` treats the un-instrumented thigh as a fixed 0° reference rather than the analysis code needing a rewrite — see `firmware/README.md`'s "Single-sensor mode" section for exactly what that assumes and where it breaks (sit-to-stand). Wiring diagram and setup steps are in [`firmware/README.md`](firmware/README.md). **Not compiled or flashed in this session** — no Arduino toolchain is available here, so it needs to be verified on real hardware before trusting it; the BLE payload math (8-byte little-endian layout) is separately unit-verified on the app side.
...
- **Flashing and field-testing the ESP32 firmware on real hardware** — this is the one piece that genuinely can't be finished without you: compile in Arduino IDE, flash, wire up per `firmware/README.md` (MPU6050 only — no LED to watch), and confirm a real BLE pairing from the app plus sensible Serial Monitor output.
